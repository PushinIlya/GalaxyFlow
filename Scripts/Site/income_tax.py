...
source_tax_name = [raw_data for raw_data in os.listdir(source) if 'Налог на прибыль' in raw_data and
                   raw_data.endswith('.xlsx')]

print(source_tax_name)
...
tax = {raw_data: pd.read_excel(os.path.join(source, f'{raw_data}'), header=None) for raw_data in source_tax_name}
...
tax_structure = {raw_data: load_workbook(filename=os.path.join(source, f'{raw_data}')) for raw_data in source_tax_name}
...
tax_structure = {i[0]: i[1].worksheets[0] for i in tax_structure.items()}
...
tax_structure = {i[0]: {row: i[1].row_dimensions[row].outline_level for row in i[1].row_dimensions} for i in tax_structure.items()}
...
tax_structure = {i[0]: pd.Series(i[1], name='Уровень').reset_index(drop=True) for i in tax_structure.items()}
...
tax = {i[0]: i[1].merge(tax_structure[i[0]], left_index=True, right_index=True, how='inner') for i in tax.items()}


# In[ ]:


tax = {i[0]: i[1].drop(list(range(6))) for i in tax.items()}
...
tax = {i[0]: i[1].rename(columns={1: 'Изменение'}) for i in tax.items()}
...
type_of_columns_tax = {'Изменение': 'float64',
                       'Организация': 'object',
                       'Счет': 'object',
                       'Кор_счет': 'object', 
                       'Вид статьи': 'object',
...
tax = {i[0]: i[1].astype(type_of_columns_tax) for i in tax.items()}
...
tax = {i[0]: i[1].drop(columns=[0, 'Кор_счет'])
       for i in tax.items()}
...
tax = {i[0]: i[1] for i in tax.items() if not i[1].empty}
...
tax = {i[0]: i[1].groupby(['Организация',
                           'Вид статьи',
                           'Статья',
                           'Дата'], as_index=False)['Изменение'].sum() for i in tax.items()}
...
for key, value in tax.items():
    ...
    if group in tax_groups:
        tax_groups[group].append(value)
    else:
        tax_groups[group] = [value]
...
tax_merged = {f'Налог на прибыль_{i[0]}.xlsx': pd.concat(i[1], axis=0).reset_index(inplace=False, drop=True)
              for i in tax_groups.items()}
...
last_date = {i[0]: i[1].sort_values(by='Дата')['Дата'].iloc[-1] for i in tax_merged.items()}
...
tax_new_rows = {i[0]: i[1].groupby(['Организация', 'Вид статьи', 'Статья'], as_index=False)['Дата'].max() for i in tax.items()}
...
for i in tax_new_rows.items():
    i[1].drop(i[1][i[1]['Дата'] == last_date[f'Налог на прибыль_{i[0].split("(")[1].split(")")[0]}.xlsx']].index, inplace=True)
...
for i in tax_new_rows.items():
    i[1]['Изменение'] = 0
    i[1]['Дата'] = last_date[f'Налог на прибыль_{i[0].split("(")[1].split(")")[0]}.xlsx']
...
tax = {i[0]: pd.concat([i[1], tax_new_rows[i[0]]]) for i in tax.items()}
...
tax = {i[0]: i[1].explode('Дата') for i in tax.items()}
...
tax = {i[0]: i[1].drop(columns=['Разница',
                                'Начальная дата',
                                'Конечная дата'])
       for i in tax.items()}
...
tax_values = {i[0]: i[1].groupby(['Организация', 'Вид статьи', 'Дата'], as_index=False)['Статья'].unique() for i in tax.items()} 
...
tax_values = {i[0]: i[1].groupby(['Организация', 'Вид статьи'], as_index=False).apply(lambda row: row.sort_values('Дата')) for i in tax_values.items()} 
...
# присвоим индексы
tax_values = {i[0]: i[1].groupby(['Организация', 'Вид статьи'], as_index=False).apply(lambda row: row.reset_index(drop=True)) for i in tax_values.items()}
...
tax_values = {i[0]: i[1].explode('Статья (доп)') for i in tax_values.items()} 


# In[ ]:


for i in tax_values.values():
...
tax_values = {i[0]: i[1].drop(columns=['Статья (доп)'])
              for i in tax_values.items()}


# In[ ]:


tax = {i[0]: pd.concat([i[1], tax_values[i[0]]]) for i in tax.items()}


# In[ ]:


tax = {i[0]: i[1].sort_values(by='Дата').groupby(['Организация',
                                                  'Вид статьи',
                                                  'Статья',
                                                  'Дата'],
                                                 as_index=False)['Изменение'].sum() for i in tax.items()}
...
tax_grouped = {i[0]: i[1].sort_values(by='Дата').groupby(['Организация',
                                                          'Вид статьи',
                                                          'Статья',
                                                          'Дата'],
                                                         as_index=False)['Изменение'].sum() for i in tax.items()}
...
for key, table1 in tax.items():
    table2 = tax_grouped.get(key)
...
tax = {i[0]: i[1].iloc[:, :-3] for i in tax.items()}
...
for i in tax.values():
    ...
    i['Показатель'] = 'Налог на прибыль'


# In[ ]:


tax = {i[0]: i[1].reset_index(drop=True) for i in tax.items()}
