...
source_oth_costs_name = [raw_data for raw_data in os.listdir(source) if 'Прочие расходы' in raw_data and
                         raw_data.endswith('.xlsx')]

print(source_oth_costs_name)
...
oth_costs = {raw_data: pd.read_excel(os.path.join(source, f'{raw_data}'), header=None) for raw_data in source_oth_costs_name}
...
oth_costs_structure = {raw_data: load_workbook(filename=os.path.join(source, f'{raw_data}')) for raw_data in source_oth_costs_name}
...
oth_costs_structure = {i[0]: i[1].worksheets[0] for i in oth_costs_structure.items()}
...
oth_costs_structure = {i[0]: {row: i[1].row_dimensions[row].outline_level for row in i[1].row_dimensions} for i in oth_costs_structure.items()}
...
oth_costs_structure = {i[0]: pd.Series(i[1], name='Уровень').reset_index(drop=True) for i in oth_costs_structure.items()}
...
oth_costs = {i[0]: i[1].merge(oth_costs_structure[i[0]], left_index=True, right_index=True, how='inner') for i in oth_costs.items()}


# In[ ]:


oth_costs = {i[0]: i[1].drop(list(range(6))) for i in oth_costs.items()}


# In[ ]:


for i in oth_costs.values():
...
oth_costs = {i[0]: i[1].rename(columns={1: 'Изменение'}) for i in oth_costs.items()}
...
type_of_columns_oth_costs = {'Изменение': 'float64',
                             'Организация': 'object',
                             'Счет': 'object',
                             'Кор_счет': 'object', 
                             'Вид статьи': 'object',
...
oth_costs = {i[0]: i[1].astype(type_of_columns_oth_costs) for i in oth_costs.items()}
...
oth_costs = {i[0]: i[1].drop(columns=[0, 'Кор_счет'])
         for i in oth_costs.items()}
...
oth_costs = {i[0]: i[1] for i in oth_costs.items() if not i[1].empty}
...
oth_costs = {i[0]: i[1].groupby(['Организация',
                                 'Вид статьи',
                                 'Статья',
                                 'Дата'], as_index=False)['Изменение'].sum() for i in oth_costs.items()}
...
for key, value in oth_costs.items():
    group = key.split('(')[1].split(')')[0]
    if group in oth_costs_groups:
        oth_costs_groups[group].append(value)
    else:
        oth_costs_groups[group] = [value]
...
oth_costs_merged = {f'Прочие расходы_{i[0]}.xlsx': pd.concat(i[1], axis=0).reset_index(inplace=False, drop=True)
                    for i in oth_costs_groups.items()}
...
last_date = {i[0]: i[1].sort_values(by='Дата')['Дата'].iloc[-1] for i in oth_costs_merged.items()}
...
oth_costs_new_rows = {i[0]: i[1].groupby(['Организация', 'Вид статьи', 'Статья'], as_index=False)['Дата'].max() for i in oth_costs.items()}
...
for i in oth_costs_new_rows.items():
    i[1].drop(i[1][i[1]['Дата'] == last_date[f'Прочие расходы_{i[0].split("(")[1].split(")")[0]}.xlsx']].index, inplace=True)
...
for i in oth_costs_new_rows.items():
    i[1]['Изменение'] = 0
    i[1]['Дата'] = last_date[f'Прочие расходы_{i[0].split("(")[1].split(")")[0]}.xlsx']
...
oth_costs = {i[0]: pd.concat([i[1], oth_costs_new_rows[i[0]]]) for i in oth_costs.items()}
...
oth_costs = {i[0]: i[1].explode('Дата') for i in oth_costs.items()}
...
oth_costs = {i[0]: i[1].drop(columns=['Разница',
                                      'Начальная дата',
                                      'Конечная дата'])
             for i in oth_costs.items()}
...
oth_costs_values = {i[0]: i[1].groupby(['Организация', 'Вид статьи', 'Дата'], as_index=False)['Статья'].unique() for i in oth_costs.items()} 
...
oth_costs_values = {i[0]: i[1].groupby(['Организация', 'Вид статьи'], as_index=False).apply(lambda row: row.sort_values('Дата')) for i in oth_costs_values.items()} 
...
# присвоим индексы
oth_costs_values = {i[0]: i[1].groupby(['Организация', 'Вид статьи'], as_index=False).apply(lambda row: row.reset_index(drop=True)) for i in oth_costs_values.items()}
...
grouped_dict = {}
for key, value in oth_costs_values.items():
    group_list = []
...
oth_costs_values = {i[0]: i[1].explode('Статья (доп)') for i in oth_costs_values.items()} 


# In[ ]:


for i in oth_costs_values.values():
...
oth_costs_values = {i[0]: i[1].drop(columns=['Статья (доп)'])
                    for i in oth_costs_values.items()}


# In[ ]:


oth_costs = {i[0]: pd.concat([i[1], oth_costs_values[i[0]]]) for i in oth_costs.items()}


# In[ ]:


oth_costs = {i[0]: i[1].sort_values(by='Дата').groupby(['Организация',
                                                        'Вид статьи',
                                                        'Статья',
                                                        'Дата'],
                                                       as_index=False)['Изменение'].sum() for i in oth_costs.items()}
...
oth_costs_grouped = {i[0]: i[1].sort_values(by='Дата').groupby(['Организация',
                                                                'Вид статьи',
                                                                'Статья',
                                                                'Дата'],
                                                               as_index=False)['Изменение'].sum() for i in oth_costs.items()}
...
grouped_dict = {}
for key, value in oth_costs_grouped.items():
...
oth_costs_grouped = grouped_dict
...
for key, table1 in oth_costs.items():
    table2 = oth_costs_grouped.get(key)
...
oth_costs = {i[0]: i[1].iloc[:, :-3] for i in oth_costs.items()}
...
for i in oth_costs.values():
    ...
    i['Показатель'] = 'Прочие расходы'


# In[ ]:


oth_costs = {i[0]: i[1].reset_index(drop=True) for i in oth_costs.items()}
...
for i in oth_costs_merged.items():
    print(f'{i[0]}:')
...
for i in source_oth_costs_name:
